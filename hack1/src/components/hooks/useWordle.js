...
const useWordle = (solution) => {
  ...
  const handleEnter = () => {
    ...
    const curGuessWithColor = [];
    const usedCharsWithColor = {};
    for (let i = 0; i < solution.length; i++) {
      if (curGuess[i] === solution[i]) {
        curGuessWithColor.push({ char: curGuess[i], color: "green" });
        usedChars[curGuess[i]] = "green";
      } else if (solution.includes(curGuess[i])) {
        curGuessWithColor.push({ char: curGuess[i], color: "yellow" });
        if (
            usedChars[curGuess[i]] !== undefined &&
            usedChars[curGuess[i]] === "green"
        ) {
            usedChars[curGuess[i]] = "green";
        ...
        //   console.log("yellow-green")
            ...
            console.log(usedChars[curGuess[i]])
            usedChars[curGuess[i]] = "yellow";
          
        //   console.log("yellow-yellow")
        }
      } else {
        curGuessWithColor.push({ char: curGuess[i], color: "grey" });
        usedChars[curGuess[i]] = "grey";
      }
    }

    guesses[turn] = curGuessWithColor;
    setGuesses(guesses);
    ...
    // const newUsedChars = Object.assign({}, usedChars, usedCharsWithColor);
